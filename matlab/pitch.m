function [ treklang ] = pitch(audio, first_pitch, second_pitch )
    ...
    n = 256; %number of samples to process at a time
    n2 = n*first_pitch;
    ...
    % Linear interpolation first pitch
    ...
    % Linear interpolation for second pitch
    ...
    omega = 2*pi*n*[0:w_size-1]'/w_size; % initiali synthetic phase variation
    phi0 = zeros(w_size,1); %previous phase variation
    first_psi = zeros(w_size,1); % the new synthetic phase variation
    ...
    while pin < pend
        ...
        meanDecibel = 10*log10(abs(mean(grain))/10e-12);
